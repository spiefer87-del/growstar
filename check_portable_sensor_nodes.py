...
def load_mqtt_service():
    ...
    spec_reg=importlib.util.spec_from_file_location("core.mqtt_sensor_devices", ROOT/"core/mqtt_sensor_devices.py")
    ...
    spec=importlib.util.spec_from_file_location("portable_mqtt", ROOT/"services/mqtt.py")
...
def main():
    ...
    pico_cfg=read("pico_sensor_02/config.py")
    ...
    require("tent_1" not in mqtt and "tent_2" not in mqtt,
            "MQTT-Empfänger ist stationsneutral")
    require('DEVICE_ID = "pico_02"' in pico_cfg and "pico_zelt" not in pico_cfg,
            "Pico-ID ist nicht an ein Zelt gekoppelt")
    require("draggable = true" in hub and "drop-zone" in hub and "pointermove" in hub,
            "Sensorhub unterstützt Desktop- und Mobile-Drag&Drop")
    require('/api/tents/${encodeURIComponent(tentId)}/sensors/assignments' in hub,
            "Drag&Drop speichert ausschließlich stationsbezogene Zuweisung")
    require('"/api/sensors/sources"' in sensors,
            "Controllerweite Sensorquellen-API vorhanden")
    require("mqtt_devices" in hardware and "list_mqtt_sensor_devices" in hardware,
            "Hardware-API liefert MQTT-Sensorcontroller")
    require("mqtt-device-grid" in devices and "MQTT Sensorcontroller" in devices,
            "Hardware-Übersicht zeigt Pico/MQTT-Sensorcontroller")

    module, registry, updates=load_mqtt_service()
    client=type("C",(),{"subscribe":lambda self, x:setattr(self,"topics",x)})()
    module.on_connect(client,None,None,0,None)
    topics={t for t,q in client.topics}
    require("growstar/sensors/+/state" in topics and "growstar/sensors/+/status" in topics,
            "Growstar empfängt State und retained Status beliebiger Sensorcontroller")

    module.on_message(None,None,Msg("growstar/sensors/pico_02/status",{
        "device_id":"pico_02","name":"Pico Sensor 2","online":True,
        "capabilities":["temperature","humidity"],"rssi":-55,
    }))
    dev=registry.get_mqtt_sensor_device("pico_02")
    require(dev and dev["online"] is True and dev["source_id"]=="mqtt:pico_02",
            "Retained Status registriert Pico controllerweit")

    module.on_message(None,None,Msg("growstar/sensors/pico_02/state",{
        "device_id":"pico_02","name":"Pico Sensor 2",
        "temperature":24.2,"humidity":51.4,"rssi":-57,
        "capabilities":["temperature","humidity"],
    }))
    require(updates[-1][0]=="mqtt:pico_02",
            "Pico erzeugt stabile stationsunabhängige source_id")
    require(registry.get_mqtt_sensor_device("pico_02")["temperature"]==24.2,
