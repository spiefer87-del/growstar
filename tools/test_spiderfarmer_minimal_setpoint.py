#!/usr/bin/env python3
"""One-off SF.4D.4 minimal Spider Farmer setpoint write experiment.

This tool does NOT use the normal Growstar device/UI write action.
It requests the bridge's private diagnostic action test_controller_minimal.

Example:
    python3 tools/test_spiderfarmer_minimal_setpoint.py \
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="SF.4D.4 minimal controller write experiment",
    )
    ...
    parser.add_argument(
        ...
        default="fan",
        choices=("fan",),
        help="SF.4D.4 test intentionally starts with fan only",
    )
...
def main():
    ...
    if len(setpoints) != 1:
        raise SystemExit(
            "Für den Minimaltest bitte genau EINEN Wert angeben: "
            "--level ODER --oscillation."
        )

    request = {
        "action": "test_controller_minimal",
