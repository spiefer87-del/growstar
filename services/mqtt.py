...
# growstar/sensors/<device_id>/state
...
def _device_id_from_topic(topic):
    ...
    if not topic.startswith(_TOPIC_PREFIX):
        return None
    if not topic.endswith(_TOPIC_SUFFIX):
        return None

    device_id = topic[len(_TOPIC_PREFIX):-len(_TOPIC_SUFFIX)]

    # Genau eine Geräte-ID zwischen Prefix und /state; keine weiteren Topics.
...
def _handle_legacy_message(topic, data):
    """Bestehende Pico-/Sensor-Topics unverändert weiter unterstützen."""
...
def _handle_generic_sensor_message(topic, data):
    ...
    Die Topic-ID ist die kanonische Geräte-ID. Ein optionales ``device_id`` im
    Payload darf sie nicht überschreiben; dadurch entstehen keine wechselnden
    source_ids durch fehlerhafte Payloads.
    """
    ...
    # Neue Feldnamen sind sprechend; temp/hum bleiben als praktische
    # Kompatibilitäts-Aliase erlaubt.
    temperature = _float_or_none(
        data.get("temperature", data.get("temp"))
    )
    humidity = _float_or_none(
...
def on_connect(client, userdata, flags, reason_code, properties):
    ...
    print(
        ...
        f"{TOPIC_DS}, {TOPIC_DHT}, {TOPIC_SENSOR_STATE}"
    )


def on_message(client, userdata, msg):
    # Controllerweiter MQTT-Traffic-Heartbeat für den Watchdog.
    ...
    # Andere Topics werden bewusst ignoriert. Der MQTT-Thread soll später auch
    # andere controllerweite MQTT-Dienste tragen können, ohne Fehler zu loggen.
