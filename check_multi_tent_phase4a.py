...
checks = {
    "Haupt-Dashboard enthält keine Zeltübersicht mehr": 'id="tent-overview-grid"' not in main_dashboard,
    ...
    "Grow-Control-Hub enthält Zeltübersicht": 'id="tent-overview-grid"' in grow_hub,
    "Grow-Control-Hub liest /api/tents": 'fetch("/api/tents"' in grow_hub,
    "Grow-Control-Hub verlinkt Default-Zelt auf Live-Regelung": "grow_control_live" in grow_hub,
    ...
    "/grow-control/live rendert bestehende Live-Ansicht": '@app.route("/grow-control/live")' in routes and 'render_template("grow_control.html")' in routes,
}

# Policy separat laden, damit der Test auch ohne installierte Growstar-App läuft.
spec = importlib.util.spec_from_file_location("phase4a_policy", ROOT / "auth" / "policy.py")
policy = importlib.util.module_from_spec(spec)
spec.loader.exec_module(policy)
req = policy.permission_requirement("/grow-control/live", "GET")
checks["Grow-Control-Unterseiten benötigen grow.view"] = (
